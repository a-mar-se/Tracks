...
def calculate_angular_velocity(df):
    ...
    for t in set(df['track']):
        ...
        for frame in range(0,len(picos_array)): # Loop over each frame
            if frame>0:   # Calcula el ángulo sólo para los frames superiores a 1
                ...
                # if len(picos_past)<=13 or len(picos_now)<=13 or len(dips_past)<=13 or len(dips_now)<=13:    
                #     angles_column.append(np.nan)
                # else:
                # try:
                    
                if len(picos_now)>14 and len(picos_past)>14 and len(dips_now)>14 and len(dips_past)>14:
                    for i in range(1,15):  
                    ...
                    if real_angle < 0:
                        ...
                        print(picos_past)
                        print('\nDiferences')
                        print(angles_reg_pic)
                        print(angles_prev_pic)
                        print(angles_next_pic)
                        ...
                        print(dips_past)
                        print('\nDiferences')
                        print(angles_reg_dip)
                        print(angles_prev_dip)
                        print(angles_next_dip)
                    # else:
                    #     angles_column.append(np.nan) 
                    # except:
                    #     angles_column.append(np.nan)
        ...
        # angles = np.array(angles_column)[indices.astype(int)]
        # for i in range(0,len(angles_column)):
        #     angles = angles_column[~np.isnan(angles_column)]
        #     # if np.isnan(angles_column[i]):
                # try:
                    # angles_column[i] = (angles_column[i-1]+angles_column[i+1])/2
                # except:
                    # angles_column = angles_column.remove(angles_column[i])
        # print(len(angles_column))
        # print(len(sub))
        sub['angular_velocity']=angles_column
        
        subs.append(sub)
        
    new = pd.concat(subs, axis=0)
    negative_values=0
    for ang in new['angular_velocity'].values:
        if ang<0:
    ...
    cleanedList = [x for x in new['angular_velocity'].values if str(x) != 'nan']

    return cleanedList

# experiment_id = '3c311b8d39f8c6b88c16d0dea0190e01'
# data_file = os.path.join( str(experiment_id)+'_extremos.pkl')
...
# ax.hist(cleanedList, bins=125)
# plt.savefig(os.path.join(str(experiment_id)+'_angles_histogram.png'), bbox_inches='tight', format='png', dpi=300)
