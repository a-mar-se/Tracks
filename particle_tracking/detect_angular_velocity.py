...
def detect_angular_velocity(experiment_id,folder):
    ...
    # lengthy = 13000 # number of rows to process
    # df = df[0:lengthy]
    ...
    # dict_indices_max_mins = dict(list(partial_get_peaks, rows), total=N)
        
    df['extremos'] = df['indice'].map(dict_indices_max_mins)
    

    df.to_pickle(os.path.join(folder, str(experiment_id)+'_extremos.pkl'), compression='xz')
